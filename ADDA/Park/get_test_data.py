...
def get_test_data(BATCH_SIZE):
    ...
    # 训练数据的构建 读取数据集中的所有数据
    test_data = R_xcsv(r'C:\Users\86182\Desktop\Spring\pytorch-adda-master\data\28x28usps\test_data.csv')
    test_Label = R_ycsv(r'C:\Users\86182\Desktop\Spring\pytorch-adda-master\data\28x28usps\test_label.csv')
    ...
    test_label = torch.tensor(test_Label, dtype=torch.long)  # csv读取int转化为long

    # 将标签和输入数据用自定义函数封装
    input_data = GetLoader(test_data, test_label)
    usps_test_data_loader = DataLoader(input_data, batch_size=BATCH_SIZE, shuffle=True)

    return usps_test_data_loader