...
#出现输入尺寸和输出尺寸不一样的时候，在自定义的生成器和判别器的forward层加入x = x.squeeze(-1)
#以达到降低一维的目的~
#不能用gpu跑程序，加上这句然后将  .cuda() 改为  .to(device)
...
class generator(nn.Module):
    ...
    def forward(self, x):
        x=self.gen(x)
        x = x.squeeze(-1)#出现target size (torch.Size([128])) that is different to the input size (torch.Size([128, 1])) is deprecated
        return x
#创建对象
D=discriminator()
G=generator()
