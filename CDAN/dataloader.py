...
def get_dataloader(dataset, batch_size, train_ratio=0.7):
    ...
    indices = torch.randperm(len(dataset))#产生随机的切片
    ...
    train_loader = DataLoader(dataset, batch_size=batch_size,
                              sampler=train_sampler, num_workers=0)#不采用多线程

    val_loader = DataLoader(dataset, batch_size=batch_size,
                            sampler=val_sampler, num_workers=0)

    # print("###################")
    return train_loader, val_loader  #返回dataloader
...
def get_office_dataloader(name_dataset, batch_size, train=True):
    ...
    root_dir = "office/%s/images" % name_dataset  #root的值为对应于office这个大文件夹里面的所有文件夹，里面的子文件夹
    ...
    data_transforms = transforms.Compose([
            transforms.Resize((224, 224)),#将输入图片转化成224×224的输入特征图
            transforms.CenterCrop(224),##依据跟定的size，从中心进行裁剪
            # transforms.RandomSizedCrop(224),
            # transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),#将图片转化为tensor
            transforms.Normalize(mean=mean_std[name_dataset]["mean"],
                                 std=mean_std[name_dataset]["std"])#对tensor进行mean和std处理
        ])
