...
def train(model, ad_net, source_loader, target_loader,
          optimizer, epoch, lambda_factor, cuda=False):#model= baseNetwork(num_classes=args.num_classes,bottleneck_dim=bottleneck_dim)
    """
    ...
    for batch_idx in range(train_steps):
        ...
        features = torch.cat((src_features, tgt_features), dim=0)#直接将源域和目标域的结果拼接
