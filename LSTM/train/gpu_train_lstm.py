...
cuda = True#False
cudnn.benchmark = True#可以让内置的 cuDNN 的 auto-tuner 自动寻找最适合当前配置的高效算法，来达到优化运行效率的问题。
...
#训练数据的构建
trainX1=R_xcsv(r'C:\Users\sensor\Desktop\YYF\Gpu\dataset\trainDigits\train0.csv')
trainX2=R_xcsv(r'C:\Users\sensor\Desktop\YYF\Gpu\dataset\trainDigitstrain1.csv')
trainX3=R_xcsv(r'C:\Users\sensor\Desktop\YYF\Gpu\dataset\trainDigits\train2.csv')
trainX4=R_xcsv(r'C:\Users\sensor\Desktop\YYF\Gpu\dataset\trainDigits\train3.csv')
trainX5=R_xcsv(r'C:\Users\sensor\Desktop\YYF\Gpu\dataset\trainDigits\train4.csv')
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(train_data):        # gives batch data
        ...
        if step % 1000 == 0:
            ...
            accuracy = float((pred_y == val_y.cpu().data.numpy()).astype(int).sum()) / float(val_y.size(0))
            #保存精度最高的模型
            if acc <= accuracy:
                torch.save(lstm, modelfilepath)  # save entire net保存整个神经网络，‘   ’保存的形式
                acc = accuracy
            print('Epoch: ', epoch, '| train loss: %.4f' % loss.cpu().data.numpy(), '| val accuracy: %.3f' % accuracy)
