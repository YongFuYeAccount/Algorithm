...
for epoch in range(EPOCH):
    ...
    modelpath = r"C:\Users\86182\Desktop\LearnText\pytorch\test\practice\LSTM\Frames\model"
    path = os.path.join(modelpath, str(epoch))
    path1 = path + '_epoch_best_lstm.pkl'  # 保存每一个处理数据的最佳模型
    path2 = path + '_epoch_final_lstm.pkl'
    path3 = path + 'epoch_acc.csv'
    ...
    for step, (b_x, b_y) in enumerate(train_data):        # gives batch data
        ...
        #     #保存精度最高的模型
        #     if acc <= accuracy:
        #         torch.save(lstm, path1)  # save entire net保存整个神经网络，‘   ’保存的形式
        #         acc = accuracy
    ...
    print('Epoch: ', epoch, '| train loss: %.4f' % loss.cpu().data.numpy(), '| val accuracy: %.3f' % acc_epoch)
    torch.save(lstm, path2)#保存每一代训练的模型
    a.append(epoch)
    b.append(acc_epoch)
    c.append(train_loss)
    d.append(val_loss)
...
dataframe.to_csv(r"C:\Users\86182\Desktop\LearnText\pytorch\test\practice\LSTM\Frames\test\frame_epoch_acc_train_val_loss.csv",index=False,sep=',')
...
# # training and testing
# #定义模型保存的绝对路径 保存有testloss的模型
# modelfilepath = r'C:\Users\86182\Desktop\LearnText\pytorch\test\practice\LSTM\Frames\model\frame_LSTM_best.pkl'
# acc = 0
# a = []
# b = []
# c = []
...
# for epoch in range(EPOCH):
#     for step, (b_x, b_y) in enumerate(train_data):        # gives batch data
#         b_x = b_x.view(-1,64 * 5 , 1 )              # reshape x to (batch, time_step, input_size)
#                                                 #rnn接收数据的形式
#         # print(b_x,type(b_x))
#         # print(val_x,type(val_x))
#         output = lstm(b_x)                               # rnn output
#         loss = loss_func(output, b_y)                   # cross entropy loss
#         optimizer.zero_grad()                           # clear gradients for this training step
#         loss.backward()                                 # backpropagation, compute gradients
#         optimizer.step()                                # apply gradients
#         torch.save(lstm, modelfilepath)

        # if step % 500 == 0:
        #     val_x = val_x.reshape(len(val_x),320,1)
        #     print("gdgggg")
        #     val_output = lstm(val_x)                   # (samples, time_step, input_size)
        #     pred_y = torch.max(val_output, 1)[1].data.numpy()
        #     print("shh")
...
# dataframe1.to_csv(r"C:\Users\86182\Desktop\LearnText\pytorch\test\practice\LSTM\Frames\test\frame_step_acc_train_val_loss.csv",index=False,sep=',')
#
# #将loss图转化为一直下降的图
# #保证后一个loss不会大于前一个
# # 绘制训练 & 验证的损失值
