...
#训练数据的构建
trainX1=R_xcsv(r'C:\Users\86182\Desktop\Dataset\dataset2\trainDigits\train0.csv')[:2000]
trainX2=R_xcsv(r'C:\Users\86182\Desktop\Dataset\dataset2\trainDigits\train1.csv')[:2000]
trainX3=R_xcsv(r'C:\Users\86182\Desktop\Dataset\dataset2\trainDigits\train2.csv')[:2000]
trainX4=R_xcsv(r'C:\Users\86182\Desktop\Dataset\dataset2\trainDigits\train3.csv')[:2000]
trainX5=R_xcsv(r'C:\Users\86182\Desktop\Dataset\dataset2\trainDigits\train4.csv')[:2000]
...
trainX = totaldata[0:8000]
train_Labels = totallable[0:8000]

valX = totaldata[8000:]
val_Labels = totallable[8000:]
...
for i in range(15,128):
    ...
    path1 = path + 'epoch_loss.csv'
    path2 = path + 'lstm_hidden.pkl'
    path3 = path + 'step_loss.csv'
    path4 = path + 'hidden_acc.csv'
    class RNN(nn.Module):
        def __init__(self):
            super(RNN, self).__init__()  # 继承module里面的属性
    ...
    for epoch in range(EPOCH):
        for step, (b_x, b_y) in enumerate(train_data):  # gives batch data
            ...
            if step % 100 == 0:
                ...
                # 保存精度最高的模型
                if acc <= accuracy:
                    torch.save(lstm, path2)  # save entire net保存整个神经网络，‘   ’保存的形式
                    acc = accuracy
    ...
    dataframe.to_csv(path1,index=False,sep=',')

    dataframe1 = pd.DataFrame({'steep':a1,'acc_step':b1,'train_step_loss':c1,'val_step_loss':d1})
    dataframe1.to_csv(path3,index=False,sep=',')

dataframe2 = pd.DataFrame({'hiddensize':hidden_1, 'acc': total_accuracy})
dataframe2.to_csv(path4, index=False, sep=',')
